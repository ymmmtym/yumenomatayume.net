...
import { getLocalBlogPosts } from '../lib/blogPosts'
...
/**
 * 外部RSSフィードから記事を取得する
 * 個人ブログはCloudflare Workers環境での自己参照フェッチ問題を避けるため、
 * ローカルファイルから直接読み込む（blogPosts.ts参照）
 */
export async function fetchExternalPosts(maxPosts: number = 10): Promise<ExternalPost[]> {
  ...
  const allPosts: ExternalPost[] = [...localPosts]
  ...
  for (const feed of RSS_FEEDS) {
    try {
      ...
      if (items.length === 0) {
        console.warn(`No items parsed for ${feed.name}`)
        continue
      }
