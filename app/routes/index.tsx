  ...
  return c.render(
    <main class="max-w-3xl mx-auto py-8 px-4">
      ...
      <section class="mb-12">
        ...
        {isFallback && (
          <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">外部記事を一時的に取得できません</p>
        )}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {externalPosts.map(post => (
            <a href={post.link} target="_blank" rel="noopener noreferrer" class={`bg-white dark:bg-purple-900/20 rounded-lg p-4 shadow hover:shadow-lg hover:-translate-y-1 transition-all block ${isFallback ? 'opacity-50' : ''}`}>
