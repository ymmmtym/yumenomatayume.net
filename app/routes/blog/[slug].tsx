        ...
        <header class="mb-8">
          ...
          <div class="flex items-center gap-2">
            ...
            {(() => {
              ...
              if (showUpdated) {
                return <span class="text-sm text-gray-600 dark:text-gray-400">(更新: {formatDate(frontmatter.updatedDate)})</span>
              }
              return null
            })()}
            {(() => {
              const { isRecent } = getUpdateStatus(frontmatter.pubDate, frontmatter.updatedDate)
              if (isRecent) {
                return <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">最近更新</span>
              }
              return null
            })()}
          </div>
            ...
            {relatedPosts.map(post => (
              <a href={`/blog/${post.slug}`} class="block p-4 bg-gray-50 dark:bg-purple-900/20 rounded-lg hover:bg-gray-100 dark:hover:bg-purple-900/30 transition-colors">
                ...
                <time class="text-xs text-gray-500 dark:text-gray-500 mt-2 block">
                  ...
                  {(() => {
                    const { showUpdated } = getUpdateStatus(post.pubDate, post.updatedDate)
                    if (showUpdated) {
                      return <span class="ml-1">(更新: {formatDate(post.updatedDate)})</span>
                    }
                    return null
                  })()}
                </time>
